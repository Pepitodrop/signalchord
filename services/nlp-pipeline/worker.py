...
def main() -> None:
    ...
    try:
        while running:
            ...
            result = extract(Document(document_id=payload["document_id"], text=payload["text"], language_hint=payload.get("language_hint")))
            ...
            graph_payload = {"stable_id": f"document:{payload['document_id']}", "entity_type": "Document", "properties": {"stable_id": payload["document_id"], "tenant_id": tenant_id, "status": "model_verified", "extraction_model": result.extraction_model, "extraction_version": result.extraction_version, "confidence": 1.0, "observed_at": source["occurred_at"]}}
