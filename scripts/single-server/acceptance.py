...
REQUIRED_STATEFULSETS = {"kafka", "postgres", "neo4j", "valkey", "minio", "opensearch"}
...
class Runner:
    ...
    def run(self, args: Sequence[str], *, input_bytes: bytes | None = None, check: bool = True) -> subprocess.CompletedProcess[bytes]:
        completed = subprocess.run(
            list(args), input=input_bytes, stdout=subprocess.PIPE, stderr=subprocess.PIPE, check=False
        )
    ...
    def kubectl(self, *args: str, input_bytes: bytes | None = None, check: bool = True) -> subprocess.CompletedProcess[bytes]:
        return self.run(["kubectl", "--namespace", self.namespace, *args], input_bytes=input_bytes, check=check)
...
def validate_namespace(runner: Runner) -> None:
    value = json.loads(
        runner.run(["kubectl", "get", "namespace", runner.namespace, "--output", "json"]).stdout.decode("utf-8")
    )
    labels = value.get("metadata", {}).get("labels", {})
    ensure(labels.get("pod-security.kubernetes.io/enforce") == "restricted", "namespace does not enforce Restricted Pod Security")
...
def validate_ingress(runner: Runner, host: str) -> None:
    ...
    for ingress in ingresses:
        ...
        if host in [rule.get("host") for rule in spec.get("rules", [])]:
            ...
            return
    raise AcceptanceError(f"no ingress rule matches {host}")
...
def ssl_context(insecure: bool) -> ssl.SSLContext:
    return ssl._create_unverified_context() if insecure else ssl.create_default_context()  # noqa: SLF001
...
<rss version=\"2.0\"><channel><title>SignalChord acceptance fixture</title>
<link>{article_url}</link><description>Repository-owned synthetic acceptance content.</description>
