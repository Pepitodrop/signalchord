...
| graph.mutation-requested.v1 | Request idempotent graph write | mutation builders | Neo4j sink | target stable_id | 30d delete | inherited | Parameterized `MERGE`; poison records to DLQ |
| graph.mutation-completed.v1 | Publish selected graph changes/CDC | Neo4j source connector | analytics, realtime, audit | changed stable_id | 30d delete | inherited | Origin/stage metadata prevents sink-source loops |
