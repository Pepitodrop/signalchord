...
class HistoryAuditUnitTest(unittest.TestCase):
    def test_allowlist_requires_complete_attestation(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            path.write_text(
                '{"schema_version":1,"allowed_binary_paths":[{"pattern":"*.mid"}],"allowed_archive_paths":[],"allowed_template_paths":[]}',
                encoding="utf-8",
            )
    ...
    def test_approved_midi_is_not_reported_as_binary(self) -> None:
        record = audit.BlobRecord("b" * 40, 4, ("velato/programs/policy.mid",))
