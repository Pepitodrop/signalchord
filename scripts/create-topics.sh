...
for topic in $TOPICS; do
  ...
  docker compose exec -T kafka "$KAFKA_TOPICS" --bootstrap-server kafka:9092 --create --if-not-exists --topic "$topic" --partitions 6 --replication-factor 1 --config cleanup.policy="$cleanup" --config retention.ms="$retention"
  docker compose exec -T kafka "$KAFKA_TOPICS" --bootstrap-server kafka:9092 --create --if-not-exists --topic "$topic.dlq" --partitions 3 --replication-factor 1 --config cleanup.policy=delete --config retention.ms=2592000000
done
