...
def validate_image_references(text: str, environment: str) -> list[str]:
    ...
    for line in image_lines:
        ...
        if MUTABLE_IMAGE.match(line):
            ...
            continue
        if environment == "production" and not DIGEST_IMAGE.match(line):
            failures.append(f"production image must use an immutable sha256 digest: {rendered}")
...
def main() -> int:
    ...
    forbidden = ["localhost", "127.0.0.1", "http://", "redis://", "neo4j://", "sha-required", "digest-required"]
    ...
    failures.extend(validate_image_references(text, args.environment))
