#!/usr/bin/env python3
"""Checksummed backup and restore for the personal single-server profile.

The supported automated contract covers PostgreSQL and the MinIO
``raw-documents`` bucket. Runtime Secret values are never exported. Kafka,
Neo4j, OpenSearch, Valkey, Prometheus and Grafana are recorded as stores that
must be rebuilt or separately recovered after a disaster.
"""
...
class CommandRunner:
    ...
    def run(
        ...
        *,
        ...
        if input_stream is not None and input_bytes is not None:
            raise RecoveryError("command input must use either a stream or bytes")
...
def app_state_from_deployments(payload: dict[str, Any]) -> dict[str, int]:
    ...
    for item in payload.get("items", []):
        ...
        if name.startswith(APP_DEPLOYMENT_PREFIX):
            state[name] = int(item.get("spec", {}).get("replicas", 1) or 0)
    if not state:
...
def current_app_state(runner: CommandRunner) -> dict[str, int]:
    return app_state_from_deployments(
        parse_json(
            runner.kubectl(
                "get",
                "deployment",
...
def backup(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        write_private(
            destination / "signalchord-values.yaml",
            runner.run(["helm", "--namespace", args.namespace, "get", "values", "signalchord", "--all"]).stdout,
        )
        write_private(
            destination / "community-values.yaml",
            runner.run(["helm", "--namespace", args.namespace, "get", "values", "signalchord-community", "--all"]).stdout,
        )
